...
// cas général, plusieurs couleurs
	for (i=1; i<=line_number; i++){
			...
			setTool("wand");
			selectWindow("image_postproc");
			waitForUser( "Pause","Selectionner la ligne avec l'outil de selection automatique et appuyez sur OK"); 
...
// pour récupérer le max de cette ligne, en première approximation
	point_array_x = newArray("point_x_array");
	point_array_y = newArray("point_y_array");
	for (i=0; i<=maxcount; i++) {
			...
			for (l=0 ; l<10; l++){
				point_x_val = (xCoordinates[j1] + ((ddX/10) * l));
				point_array_x_temp = newArray(lengthOf(point_array_x)+1);
				
				for (m=0; m<lengthOf(point_array_x);m++){
					point_array_x_temp[m]=point_array_x[m];
				}
				
				point_array_x_temp[lengthOf(point_array_x_temp)-1]=point_x_val;
				point_array_x = point_array_x_temp;

				point_y_val = (yCoordinates[j1] + ((ddY/10) * l));
				point_array_y_temp = newArray(lengthOf(point_array_y)+1);
				
				for (n=0; n<lengthOf(point_array_y);n++){
					point_array_y_temp[n]=point_array_y[n];
				}
				
				point_array_y_temp[lengthOf(point_array_y_temp)-1]=point_y_val;
				point_array_y = point_array_y_temp;
				
			}
	}
	
	point_array_x = Array.slice(point_array_x,1);
	point_array_y = Array.slice(point_array_y,1);
	p_count = 0;
	while (p_count < (line_number)){
		for (p=((p_count*10)) ; p<(10 + (p_count*10)); p++){
			if (p == (line_number*10)){
			print("PTX : " + point_array_x[p-1] + " PTY : " + point_array_y[p-1]);
			makePoint(point_array_x[p-1], point_array_y[p-1]);
			}
			else {
				print("PTX : " + point_array_x[p] + " PTY : " + point_array_y[p]);
				makePoint(point_array_x[p], point_array_y[p]);
			}
	...
	for (q=0; q<line_number; q++){
		for (r=1; r<=9; r++){
			...
			Overlay.drawLine(point_array_x[s+r] , point_array_y[s+r] , point_array_x[t+r] , point_array_y[t+r]);
			run("Measure");
		}
	}
